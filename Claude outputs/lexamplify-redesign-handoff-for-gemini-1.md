...
## 12. Latest round — Home Gateway v3 (logo, context-trail redesign, default-naming fix)

Triggered by JN sending the real LexAmplify logo file and a screenshot of the live capsule reading "My 1st Matter · My Private Space · Firm Console." Delivered as `home-gateway-v3.html` / `home-gateway-v3-brief.md` (builds on v2, doesn't replace it) plus `assets/lexamplify-logo-mark.png`. Summary:

- **Logo mark added** to the sidebar head, before the firm name — a real brand asset (`assets/lexamplify-logo-mark.png`), not a hand-built SVG icon like the rest of the icon system, since a logo has to reproduce exact brand artwork. Reads cleanly in both themes.
- **Context capsule redesigned** from three identical, unranked pill chips into a connected "trail": the current level (almost always the active matter) is solid-rust-filled, the other two are ghost/ranked behind it, `›` chevrons between levels, and — fixing an inconsistency in the old design — all three segments now carry the same dropdown affordance (the old capsule only gave the first two a chevron).
- **Default seed data renamed again, for a confirmed reason:** "My 1st Matter" is Lexlegis.ai's own literal default trial-account matter name (confirmed from the same recording reviewed in the v2 round) — changed to `"Untitled Matter"` in `useOrganizationStore.js`. "Firm Console" was already clear of this problem (Lexlegis's own equivalent segment is literally "Org Dashboard," already avoided last round) and only got the visual redesign, not another rename.
- **A live defect caught from JN's own screenshot, not hypothetical:** the v2 mockup's hero-banner implementer note ("— every number in this line is computed live from...") had shipped to the real site verbatim, styled closely enough to real copy that whoever implemented it didn't catch it. Fixed in the mockup by moving all such notes into a visually distinct dashed-border "SPEC NOTE" callout that cannot be mistaken for product copy — and flagged as a reason to re-check anything already built from `home-gateway-v2.html` for the same mistake.
- **Adjacent bug noted, not required for this round:** the seed matter's `leadCounsel` field is hardcoded to `'Narendar V'` in the store rather than read from the authenticated user — same file as the naming fix, worth doing at the same time.
- **Not yet applied to the live repo** — mockup + brief only, same pattern as every round.
...
- `home-gateway-v3.html` + `home-gateway-v3-brief.md` + `assets/lexamplify-logo-mark.png`
- This document
