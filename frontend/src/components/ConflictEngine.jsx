...
const styles = `
  ...
  /* ── Hydration skeleton (Architect's Innovation) ── */
  /* Plays for 420ms when restoring a historical conflict report,
     preventing the jarring snap from dropzone → full results panel. */
  .hydration-skeleton-wrap {
    animation: hydration-enter 0.22s cubic-bezier(0.16,1,0.3,1);
    padding: 4px 0;
  }
  @keyframes hydration-enter {
    from { opacity: 0; transform: translateY(10px); }
    to   { opacity: 1; transform: translateY(0); }
  }
  .hydration-banner {
    display: flex; align-items: center; gap: 12px;
    padding: 14px 20px; border-radius: 10px; margin-bottom: 22px;
    background: var(--accent-muted);
  ...
  .legacy-notice {
    ...
    border-left: 3px solid var(--badge-warn-color);
    border-radius: 8px; padding: 12px 16px; margin-bottom: 18px;
    font-size: 12.5px; color: var(--badge-warn-color); line-height: 1.55;
  }
...
export default function ConflictEngine() {
  ...
  // Guards async continuations (post-await / post-setTimeout state updates)
  // from firing after this component has unmounted.
  ...
  useEffect(() => {
    try {
      const saved = sessionStorage.getItem('lexai_crossResults');
      if (saved) {
        const parsed = JSON.parse(saved);
        setCrossResults(parsed);
        if (Array.isArray(parsed._fileLabels) && parsed._fileLabels.length > 0) {
          setSessionFiles(parsed._fileLabels.map(name => ({ name, size: 0 })));
        }
      }
    } catch {}
  }, []);
...
`;
  ...
  const persistReport = (result, files) => {
    const entry = {
      ...
      // Full payload stored for one-click workspace hydration
      fullResult: result,
    };
  ...
  const handleCrossDocAnalyze = async () => {
    ...
    if (res.error) {
      ...
      // Embed file labels so the results panel can display names after hydration
      const enriched = { ...res, _fileLabels: sessionFiles.map(f => f.name) };
      setCrossResults(enriched);
      persistReport(enriched, sessionFiles);
      // Survive navigation — restored in the mount useEffect
      try { sessionStorage.setItem('lexai_crossResults', JSON.stringify(enriched)); } catch {}
    }
  };
  ...
  //   The JSX branches on `!crossResults` — if crossResults is null/undefined
  //   the dropzone renders; any truthy value flips to the results grid.
  //   Root cause of the stuck-on-dropzone bug: the old handler only called
  //   setCrossResults when r.fullResult was truthy. For LEGACY entries (saved
  //   before fullResult was added to persistReport) that branch was never taken,
  //   so crossResults stayed null and the view never switched.
  //
  //   Fix: ALWAYS build a displayable result object. For legacy entries, construct
  //   a safe fallback stub so the view switches cleanly and the legacy-notice banner
  //   explains that full clause-level data requires a re-run.
  const hydrateFromReport = (r) => {
    // If this report is already showing in the results panel, second click resets to dropzone
    if (selectedReport?.id === r.id && crossResults) {
      resetCrossDoc();
      return;
    }
    ...
    // Always produce a renderable result — fall back to a safe stub for legacy entries
    const resultToLoad = r.fullResult ?? {
      summary: r.summary || 'This report was saved in a previous session. Re-run the analysis with the original documents to restore full clause-level data.',
      conflicts: [],
      _fileLabels: r.fileNames || [],
      _isLegacy: true,
    };
    const virtualFiles = (r.fileNames || []).map(name => ({ name, size: 0 }));

    // Architect's Innovation: 420ms "Restoring workspace" skeleton before snapping
    // to the results panel — prevents jarring instant-jump from dropzone to full UI.
    setIsHydrating(true);
    setTimeout(() => {
      if (!isMountedRef.current) return;
      setAddingToSet(false);
      setSessionFiles(virtualFiles);
      // This is the decisive state update — crossResults becoming truthy flips the
      // JSX branch from dropzone → results grid.
      setCrossResults(resultToLoad);
      try { sessionStorage.setItem('lexai_crossResults', JSON.stringify(resultToLoad)); } catch {}
      setIsHydrating(false);
      showToastMsg(
        r.fullResult ? '📂 Conflict report restored to workspace' : '⚠️ Legacy report — re-run analysis for full clause view.',
        !r.fullResult,
      );
    }, 420);
  };
    ...
    <>
      ...
      <div className="conflict-container p-4 md:p-8">
        ...
        {sharedFiles.length > 0 && (
          <div style={{ marginBottom: '20px', background: 'rgba(139,92,246,0.05)', border: '1px solid rgba(139,92,246,0.2)', borderRadius: '8px', overflow: 'hidden' }}>
            <div style={{ padding: '10px 16px', borderBottom: '1px solid rgba(139,92,246,0.15)', display: 'flex', alignItems: 'center', gap: '7px' }}>
              ...
              <span style={{ fontSize: '11px', fontWeight: 700, color: '#A78BFA', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Documents shared from platform — {sharedFiles.length}</span>
            </div>
            {sharedFiles.map(f => (
              <div key={f.id} style={{ padding: '10px 16px', borderBottom: '1px solid rgba(139,92,246,0.08)', display: 'flex', alignItems: 'center', gap: '10px' }}>
        ...
        {activeMode === 'cross-doc' && (
          <div style={{ display: 'flex', flexDirection: 'column' }}>
            {isHydrating ? (
              /* ── Restoring Workspace Skeleton (Architect's Innovation) ── */
              <div className="hydration-skeleton-wrap">
                <div className="hydration-banner">
                  <div className="loading-spinner" style={{ width: '18px', height: '18px', flexShrink: 0 }} />
                  <span>Restoring conflict workspace…</span>
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: '0.35fr 0.65fr', gap: '24px' }}>
                  <div className="hydration-card">
                    <div className="skeleton-bar" style={{ height: '20px', width: '55%', marginBottom: '18px' }} />
                    <div className="skeleton-bar" style={{ width: '100%', marginBottom: '10px' }} />
                    <div className="skeleton-bar" style={{ width: '80%', marginBottom: '10px' }} />
                    <div className="skeleton-bar" style={{ width: '65%', marginBottom: '22px' }} />
                    <div style={{ padding: '16px', background: 'var(--stat-bg)', border: '1px solid var(--stat-border)', borderRadius: '8px', marginBottom: '18px', textAlign: 'center' }}>
                      <div className="skeleton-bar" style={{ height: '36px', width: '40%', margin: '0 auto 10px' }} />
                      <div className="skeleton-bar" style={{ width: '55%', margin: '0 auto' }} />
                    </div>
                    <div className="skeleton-bar" style={{ height: '34px', width: '100%', marginBottom: '8px', borderRadius: '8px' }} />
                    <div className="skeleton-bar" style={{ height: '34px', width: '100%', borderRadius: '8px' }} />
                  </div>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                    <div className="skeleton-bar" style={{ height: '24px', width: '45%' }} />
                    {[85, 70, 90].map((w, i) => (
                      <div key={i} className="hydration-card" style={{ padding: 0 }}>
                        <div style={{ padding: '14px 20px', borderBottom: '1px solid var(--border-dark-subtle)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                          <div className="skeleton-bar" style={{ width: `${w}%`, height: '15px' }} />
                          <div className="skeleton-bar" style={{ width: '58px', height: '22px', marginLeft: '14px', borderRadius: '20px' }} />
                        </div>
                        <div style={{ padding: '16px 20px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                          <div className="skeleton-bar" style={{ height: '66px', borderRadius: '8px' }} />
                          <div className="skeleton-bar" style={{ height: '66px', borderRadius: '8px' }} />
                        </div>
                      </div>
                    ))}
              ...
              <>
                ...
                {recentReports.length > 0 && !isLoading && (
                  <div className="recent-reports">
                    ...
                    {/* All reports now hydrate the full results panel via hydrateFromReport;
                        the legacy summary panel is retired. */}
                  </div>
                )}
              </>
            ) : (
              ...
              <div className="cross-results-grid">
                <div className="summary-col">
                  <div className="summary-card">
                    ...
                    {crossResults._isLegacy && (
                      <div className="legacy-notice">
                        <span style={{ flexShrink: 0 }}>⚠️</span>
                        <span>Legacy report — clause-level conflict cards are not available for this entry. Re-run the analysis with the original documents to restore the full interactive view.</span>
                      </div>
                    )}
                    ...
                    <ul style={{ paddingLeft: '20px', margin: '8px 0 20px 0', fontSize: '13px', color: 'var(--text-dark-primary)', display: 'flex', flexDirection: 'column', gap: '6px' }}>
                      {(sessionFiles.length > 0 ? sessionFiles.map(f => f.name) : (crossResults._fileLabels || [])).map((name, i) => (
                        <li key={i}>📂 {name}</li>
                      ))}
                    </ul>
