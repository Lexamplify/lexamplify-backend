...
`;
  ...
  const hydrateFromReport = (r) => {
    // Second click on the same row — deselect and stay on dropzone
    if (selectedReport?.id === r.id && !r.fullResult) {
      setSelectedReport(null);
      return;
    }
    setSelectedReport(r);
    if (r.fullResult) {
      setAddingToSet(false);
      setError('');
      // Virtual file objects — only `.name` is needed for the results display
      const virtualFiles = (r.fileNames || []).map(name => ({ name, size: 0 }));
      setSessionFiles(virtualFiles);
      setCrossResults(r.fullResult);
      try { sessionStorage.setItem('lexai_crossResults', JSON.stringify(r.fullResult)); } catch {}
      showToastMsg('📂 Historical report restored to workspace');
    }
    // Legacy entries without fullResult just toggle the summary panel (no view switch)
  };
              ...
              <>
                ...
                {recentReports.length > 0 && !isLoading && (
                  <div className="recent-reports">
                    ...
                    {/* Legacy summary panel — only shown for entries without a full payload */}
                    {selectedReport && !selectedReport.fullResult && (
