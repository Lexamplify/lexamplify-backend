...
import bailData from '../data/delhi_rohini_north_bail_roster.json';
import leaveData from '../data/delhi_rohini_north_leave.json';
import { useJudgesDirectory } from '../hooks/useJudgesDirectory';
import styles from './RohiniCourtDirectory.module.css';

export default function RohiniCourtDirectory() {
  const { judges: judgesData, isLoading, error, refetch } = useJudgesDirectory('delhi_rohini');
  ...
  const filteredBail = useMemo(() => {
    ...
    return bailData.filter((b) => {
      ...
      const matchRoom = b.court_room?.toLowerCase().includes(q) || false;
      return matchPS || matchJudge || matchRoom;
    });
  }, [searchTerm]);
  ...
  return (
    <div className={styles.container}>
      ...
      <header className={styles.header}>
        <div>
          <div className={styles.badgeRow}>
            <span className={styles.districtBadge}>North District</span>
            <span className={styles.complexBadge}>Rohini Court Complex</span>
          </div>
          <h2 className={styles.title}>Rohini District Courts — Judicial Roster & Rosters</h2>
      ...
      {/* Search Bar (Shown for searchable folders) */}
      {activeFolder !== 'duty' && activeFolder !== 'leave' && (
        <div className={styles.searchWrapper}>
          ...
          <input
            ...
            placeholder={
              activeFolder === 'bail'
                ? "Search by Police Station, Judge, or Court Room..."
          ...
          {isLoading ? (
            ...
            <div key={j.id} className={styles.card}>
              ...
              <div className={styles.metaBox}>
                <div className={styles.metaRow}>
                  ...
                  <button
                    ...
                    onClick={() => copyText(j.vc_meeting_id, `${j.id}-id`)}
                  >
              ...
              <div className={styles.cardActions}>
                <a href={j.vc_link} target="_blank" rel="noreferrer" className={styles.joinBtn}>
                  Join Virtual Court
                </a>
                <button
                  className={styles.copyBtn}
                  onClick={() => copyText(j.vc_link, `${j.id}-link`)}
                >
            ...
            <div key={b.id} className={styles.bailCard}>
              ...
              <div className={styles.linkJudgesContainer}>
                <div className={styles.linkBox}>
                  <span className={styles.linkTitle}>1st Link Judge (In Absence):</span>
      ...
      {/* FOLDER 4: DUTY MAGISTRATE ROSTER PDF */}
      {activeFolder === 'duty' && (
        <div className={styles.pdfContainer}>
          <div className={styles.pdfHeaderActions}>
            <div>
              <h3 className={styles.pdfTitle}>Official Duty Magistrate Monthly Order — North District</h3>
              <p className={styles.pdfSubtitle}>Official roster issued by the Chief Metropolitan Magistrate, North District</p>
            </div>
            <a
              href={dutyMagistratePdfUrl}
              target="_blank"
              rel="noreferrer"
          ...
          <iframe
            src={`${dutyMagistratePdfUrl}#toolbar=0`}
