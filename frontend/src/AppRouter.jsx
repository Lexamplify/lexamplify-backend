...
const Layout = ({ children, focusMode, setFocusMode }) => {
  ...
  const handleOpenSearch = () => {
    // Fire global decoupled custom event to trigger self-contained CommandPalette
    ...
    <div className={`app-container ${focusMode ? 'focus-mode-active' : ''}`}>
      ...
      <aside className="sidebar">
        ...
        <nav style={{ flex: 1, padding: '20px 0' }}>
          <Link to="/dashboard" style={{ display: 'block', padding: '12px 24px', color: location.pathname === '/dashboard' ? 'var(--accent-primary)' : 'var(--text-primary)', textDecoration: 'none', fontSize: '14px' }}>
            📊 Dashboard Home
          </Link>

          <Link to="/contract-analyzer" style={{ display: 'block', padding: '12px 24px', color: location.pathname === '/contract-analyzer' ? 'var(--accent-primary)' : 'var(--text-primary)', textDecoration: 'none', fontSize: '14px' }}>
            📄 Contract Analyzer
          </Link>

          <Link to="/court-resources" style={{ display: 'block', padding: '12px 24px', color: location.pathname === '/court-resources' ? 'var(--accent-primary)' : 'var(--text-primary)', textDecoration: 'none', fontSize: '14px' }}>
            ⚖️ Court Resources
          </Link>

          <Link to="/conflict-engine" style={{ display: 'block', padding: '12px 24px', color: location.pathname === '/conflict-engine' ? 'var(--accent-primary)' : 'var(--text-primary)', textDecoration: 'none', fontSize: '14px' }}>
            🔍 Conflict Engine
          </Link>

          <Link to="/calendar" style={{ display: 'block', padding: '12px 24px', color: location.pathname === '/calendar' ? 'var(--accent-primary)' : 'var(--text-primary)', textDecoration: 'none', fontSize: '14px' }}>
            📅 Legal Calendar
          </Link>

          <Link to="/vault" style={{ display: 'block', padding: '12px 24px', color: location.pathname === '/vault' ? 'var(--accent-primary)' : 'var(--text-primary)', textDecoration: 'none', fontSize: '14px' }}>
            🗄️ Case Vault
          </Link>

          <Link to="/war-room" style={{ display: 'block', padding: '12px 24px', color: location.pathname === '/war-room' ? 'var(--accent-primary)' : 'var(--text-primary)', textDecoration: 'none', fontSize: '14px' }}>
            🏛️ Virtual Courtroom
          </Link>
          ...
          {Object.values(DEMO_CASES).map(c => (
            <Link key={c.id} to={`/case/${c.id}`} style={{ display: 'block', padding: '8px 24px', color: params.caseId === c.id ? 'var(--accent-primary)' : 'var(--text-muted)', textDecoration: 'none', fontSize: '13px' }}>
              📁 {c.title}
            </Link>
          ))}
        </nav>
        ...
        <div style={{ padding: '20px', borderTop: '1px solid var(--border-subtle)', display: 'flex', flexDirection: 'column', gap: '10px' }}>
          <button onClick={handleOpenSearch} style={{ width: '100%', padding: '8px', background: 'var(--bg-panel)', color: 'var(--text-primary)', border: '1px solid var(--border-subtle)', borderRadius: '6px', cursor: 'pointer', fontSize: '12px' }}>
            🔍 RAG Chat (Ctrl+K)
          </button>
